-- The admin dashboard's "Capacity" gauge was multiplying listing count by
-- an assumed average photo count and size (listingsTotal * 2 * 300KB) --
-- a guess, not a measurement. storage.objects.metadata already carries the
-- real byte size of every uploaded file, so this replaces the guess with
-- an actual sum. Deliberately NOT scoped to target_college: the 1GB ceiling
-- is a project-wide Supabase plan limit shared across every college, not a
-- per-tenant quota.
