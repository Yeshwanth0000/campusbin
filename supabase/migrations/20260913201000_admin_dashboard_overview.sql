-- The Power-BI-style dashboard's single data source: time-bucketed series
-- (day/week/month/year), period-over-period deltas, and a per-college
-- breakdown for the platform owner. One function rather than one-per-role
-- because the access rule is the interesting part and it belongs in one
-- place: a superadmin may pick any college or none (= every college
-- combined); a plain college admin is pinned to their own college no
-- matter what they pass in, so this can never leak another campus's data
-- to a future per-college admin.
...
begin
  ...
  -- Time series, bucketed by the requested granularity. Small dataset, so
  -- correlated subqueries per bucket are fine -- no need for a window-
  -- function rewrite at this scale.
  buckets as (
    select gs.bucket
